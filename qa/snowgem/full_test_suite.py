#!/usr/bin/env python2
#
# Execute all of the automated tests related to Snowgem.
#
...
def test_rpath_runpath(filename):
    output = subprocess.check_output(
        [repofile('qa/snowgem/checksec.sh'), '--file', repofile(filename)]
    )
...
def test_fortify_source(filename):
    proc = subprocess.Popen(
        [repofile('qa/snowgem/checksec.sh'), '--fortify-file', repofile(filename)],
...
def check_security_hardening():
    ...
    ret &= test_rpath_runpath('src/snowgemd')
    ret &= test_rpath_runpath('src/snowgem-cli')
    ret &= test_rpath_runpath('src/snowgem-gtest')
    ret &= test_rpath_runpath('src/snowgem-tx')
    ret &= test_rpath_runpath('src/test/test_bitcoin')
    ret &= test_rpath_runpath('src/snowgem/GenerateParams')

    # NOTE: checksec.sh does not reliably determine whether FORTIFY_SOURCE
    # is enabled for the entire binary. See issue #915.
    ret &= test_fortify_source('src/snowgemd')
    ret &= test_fortify_source('src/snowgem-cli')
    ret &= test_fortify_source('src/snowgem-gtest')
    ret &= test_fortify_source('src/snowgem-tx')
    ret &= test_fortify_source('src/test/test_bitcoin')
    ret &= test_fortify_source('src/snowgem/GenerateParams')
...
STAGE_COMMANDS = {
    ...
    'gtest': [repofile('src/snowgem-gtest')],
