...
public:
    ...
    virtual bool AddSpendingKey(const libsnowgem::SpendingKey &sk) =0;
...
public:
    ...
    bool GetSpendingKey(const libsnowgem::PaymentAddress &address, libsnowgem::SpendingKey &skOut) const
    {
        {
            LOCK(cs_SpendingKeyStore);
    ...
    bool GetNoteDecryptor(const libsnowgem::PaymentAddress &address, ZCNoteDecryption &decOut) const
    {
        {
            LOCK(cs_SpendingKeyStore);
    ...
    void GetPaymentAddresses(std::set<libsnowgem::PaymentAddress> &setAddress) const
    {
        setAddress.clear();
        {
            LOCK(cs_SpendingKeyStore);
...
typedef std::map<libsnowgem::PaymentAddress, std::vector<unsigned char> > CryptedSpendingKeyMap;
