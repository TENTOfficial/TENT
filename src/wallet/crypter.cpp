...
static bool DecryptSpendingKey(const CKeyingMaterial& vMasterKey,
                               ...
                               libsnowgem::SpendingKey& sk)
{
    CKeyingMaterial vchSecret;
    if(!DecryptSecret(vMasterKey, vchCryptedSecret, address.GetHash(), vchSecret))
        return false;

    if (vchSecret.size() != libsnowgem::SerializedSpendingKeySize)
        return false;
...
{
    {
        ...
        {
            const libsnowgem::PaymentAddress &address = (*skmi).first;
...
bool CCryptoKeyStore::AddSpendingKey(const libsnowgem::SpendingKey &sk)
{
    {
...
bool CCryptoKeyStore::AddCryptedSpendingKey(const libsnowgem::PaymentAddress &address,
...
bool CCryptoKeyStore::GetSpendingKey(const libsnowgem::PaymentAddress &address, libsnowgem::SpendingKey &skOut) const
{
    {
        LOCK(cs_SpendingKeyStore);
        if (!IsCrypted())
...
{
    {
        ...
        {
            const libsnowgem::SpendingKey &sk = mSpendingKey.second;
            CSecureDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
            ss << sk;
            CKeyingMaterial vchSecret(ss.begin(), ss.end());
            libsnowgem::PaymentAddress address = sk.address();
            std::vector<unsigned char> vchCryptedSecret;
