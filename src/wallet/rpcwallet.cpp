...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    ...
    if (params.size() > 2) {
        ...
        for (size_t idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Snowgem address: ")+input.get_str());
            if (setAddress.count(address))
...
UniValue zc_sample_joinsplit(const UniValue& params, bool fHelp)
{
    ...
    JSDescription samplejoinsplit(*psnowgemParams,
                                  pubKeyHash,
                                  anchor,
                                  {JSInput(), JSInput()},
                                  {JSOutput(), JSOutput()},
...
UniValue z_getnewaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 0)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "\nResult:\n"
            "\"snowgemaddress\"    (string) The new zaddr\n"
            "\nExamples:\n"
...
UniValue z_listreceivedbyaddress(const UniValue& params, bool fHelp)
{
    ...
    libsnowgem::PaymentAddress zaddr;
...
UniValue z_shieldcoinbase(const UniValue& params, bool fHelp)
{
    ...
    try {
        ...
        libsnowgem::PaymentAddress zaddr = pa.Get();
    } catch (const std::runtime_error&) {
