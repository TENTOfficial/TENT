...
CWalletTx GetValidReceive(const libsnowgem::SpendingKey& sk, CAmount value, bool randomInputs) {
    return GetValidReceive(*params, sk, value, randomInputs);
}

libsnowgem::Note GetNote(const libsnowgem::SpendingKey& sk,
                       const CTransaction& tx, size_t js, size_t n) {
    return GetNote(*params, sk, tx, js, n);
}

CWalletTx GetValidSpend(const libsnowgem::SpendingKey& sk,
...
TEST(wallet_tests, FindMyNotes) {
    ...
    auto sk = libsnowgem::SpendingKey::random();
    auto sk2 = libsnowgem::SpendingKey::random();
