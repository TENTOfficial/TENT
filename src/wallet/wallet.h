...
{
public:
    libsnowgem::PaymentAddress address;
    ...
    CNoteData(libsnowgem::PaymentAddress a) :
            address {a}, nullifier(), witnessHeight {-1} { }
    CNoteData(libsnowgem::PaymentAddress a, uint256 n) :
...
{
    JSOutPoint jsop;
    libsnowgem::NotePlaintext plaintext;
};
...
public:
    ...
    std::map<libsnowgem::PaymentAddress, CKeyMetadata> mapZKeyMetadata;
    ...
    bool AddZKey(const libsnowgem::SpendingKey &key);
    //! Adds spending key to the store, without saving it to disk (used by LoadWallet)
    bool LoadZKey(const libsnowgem::SpendingKey &key);
