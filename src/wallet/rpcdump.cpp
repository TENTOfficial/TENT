    ...
    while (file.good()) {
        ...
        if (fImportZKeys) {
            try {
                ...
                libsnowgem::SpendingKey key = spendingkey.Get();
                libsnowgem::PaymentAddress addr = key.address();
                ...
                // Not a valid spending key, so carry on and see if it's a Snowgem style address.
            }
        }
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey \"snowgemaddress\"\n"
            "\nReveals the private key corresponding to 'snowgemaddress'.\n"
            "Then the importprivkey can be used with this output\n"
            "\nArguments:\n"
            "1. \"snowgemaddress\"   (string, required) The snowgem address for the private key\n"
            "\nResult:\n"
